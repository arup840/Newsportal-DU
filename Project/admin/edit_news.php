...
if(isset($_POST['add_news'])) {
	...
	try {
		...
		if(empty($_FILES["image"]["name"])) {
			...
			$statement = $db->prepare("UPDATE news SET title=?, long_description=?, faculty_id=?, category_id=? WHERE news_id=?");
			$statement->execute(array($_POST['title'],$_POST['long_description'],$_POST['faculty_id'],$_POST['category_id'],$id));
			...
			$statement = $db->prepare("UPDATE news SET title=?, long_description=?,image=?, faculty_id=?, category_id=? WHERE news_id=?");
			$statement->execute(array($_POST['title'],$_POST['long_description'],$f1,$_POST['faculty_id'],$_POST['category_id'],$id));
...
<form class="form-horizontal" method="post" enctype="multipart/form-data">
     ...
     <table class="">
        ...
        <tr>
            ...
            <td>
			<textarea class="form-control textarea" name="long_description" cols="30" rows="10"><?php echo $long_description; ?></textarea>
					...
					<option value="">Select a Faculty</option>
						<?php
							...
							foreach($result as $row)
							{
								...
								{
									?><option value="<?php echo $row['faculty_id']; ?>"selected><?php echo $row['short_name']; ?></option><?php
								}
								else
								{
									?><option value="<?php echo $row['faculty_id']; ?>"><?php echo $row['short_name']; ?></option><?php
								}
							}
						?>
				</select>
					...
					<option value="">Select a Category</option>
						<?php
							...
							foreach($result as $row)
							{
								...
								{
									?><option value="<?php echo $row['category_id']; ?>"selected><?php echo $row['cat_name']; ?></option><?php
								}
								else
								{
									?><option value="<?php echo $row['category_id']; ?>"><?php echo $row['cat_name']; ?></option><?php
								}
							}
						?>
				</select>
		...
		<tr>
			<th class="bold">Previous Featured Image Preview</th>
		
		
			<td class=""><img src="../uploads/<?php echo $image; ?>" alt="" width="200"></td>
		</tr>
		<tr>
        ...
        <tr>
            <th></th>
            <td class="bold"><input type="submit" class="btn btn-primary" name="add_news" value="Edit News"</td>
        </tr>
    </table>
</form>
